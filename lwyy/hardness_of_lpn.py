...
def Gauss(N, k, t):
    ...
    # T = the cost of inverting a matrix via Strassenâ€™s algorithm
    if N - k > k:
        T = pow(k, 2.8)
    else:
        T = pow(N - k, 2.8)
...
def min_sub_BJMM_ISD(N, k, t, p2, l):
    ...
    # We focus on the LPN problem whose bit security is smaller than 999
    localmin = 999
...
if __name__ == '__main__':
    ...
    k = r
    t = 421
    print(f"sqrt speed up={math.log2(math.sqrt(k))}")
    #analysisfor2(N, k, t)
    ...
    v = 55
    w = 3*v
    print(f"linear speed up={math.log2(r)}")


    # res = AGBforq(2**9,2**8,152)
    # print(f"res={res}")
    # res = AGBforq(2**10,3*2**8,64)
    # print(f"res={res}")
    #
    # res = Gauss(2**9,2**8,152)
    # print(f"res={res}")
    # res = Gauss(2**10,3*2**8,64)
    # print(f"res={res}")
    #main()
